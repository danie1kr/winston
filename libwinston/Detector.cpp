...
namespace winston
{
	...
	DetectorDevice::DetectorDevice(const std::string name, Callbacks::LocoFromAddress locofromAddressCallback)
		: Shared_Ptr<DetectorDevice>(), name(name), locofromAddressCallback(locofromAddressCallback)
	{
	...
	const Result DetectorDevice::change(const size_t port, const Address locoAddress, const Detector::Change change)
	{
		if (this->ports.find(port) != this->ports.end())
		{
			...
			auto loco = this->locofromAddressCallback(locoAddress);
