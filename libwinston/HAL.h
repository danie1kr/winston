...
namespace winston
{
	namespace hal
	{
		...
		public:
			...
			const Result readUint32(const size_t address, uint32_t& content);
			const Result readUint16(const size_t address, uint16_t& content);
			const Result readFloat(const size_t address, float& content);
			virtual const Result writeVector(const size_t address, const std::vector<unsigned char>& content, const size_t length = 0) = 0;
			virtual const Result writeString(const size_t address, const std::string& content, const size_t length = 0) = 0;
			virtual const Result write(const size_t address, const unsigned char content) = 0;
			const Result writeUint32(const size_t address, const uint32_t content);
			const Result writeUint16(const size_t address, const uint16_t content);
			const Result writeFloat(const size_t address, const float content);
